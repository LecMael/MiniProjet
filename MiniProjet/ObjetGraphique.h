...
public://Toutes ces méthodes ont été réalisées par MOHAMAN BELLO Souley
	...
	bool victoire(sf::Sprite* concurrent, sf::Sprite* objectif) const { return concurrent->getPosition().x + 30 > objectif->getPosition().x; };//Cette méthode renvoie un booléen qui indique si un coureur a franchi la ligne d'arrivée ou pas. Elle prend en paramètre un pointeur sur le sprite du joueur considéré et un pointeur sur le sprite de l'objectif, ici la ligne d'arrivée.
};