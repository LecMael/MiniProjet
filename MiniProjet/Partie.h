...
public:
	...
	void Sauver(ofstream &ofs, string path) const;//Sauvegarde les données dans des fichiers txt, par Maël LECOEUCHE
	bool Deroulement();//Cette méthode gère le dessin de la fenêtre, la création et le dessin des sprites et la gestion des évènements clavier et souris, par MOHAMAN BELLO Souley 
	...
	void timer(int duree);//cette méthode réalise une boucle permettant de faire passer duree millisecondes, par MOHAMAN BELLO Souley
	int surbrillance(sf::Sprite* snipperSprite);//cette méthode permet d'indiquer à travers l'indice à la fonction déroulement si le snipper à en viser un coureur, afin qu'il soit affiché en rouge plus tard dans déroulement. Elle prend en paramètre un pointeur sur le sprite du snipper. Par MOHAMAN BELLO Souley.
};
